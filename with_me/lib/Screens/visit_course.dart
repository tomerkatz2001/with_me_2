...
class _VisitCoursePageState extends State<VisitCoursePage> {
  ...
  Widget visitsCourse(List<Station> stations,int current_index){
    ...
    List<Widget> texts = [];
    texts.add(Column(children:[Text(stations[0].name),Text(stations[0].time)]));
    int c=0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: CircularAppBar("הביקור שלי", [
        Container(child:Positioned(child: SingleChildScrollView(child:
        visitsCourse([
